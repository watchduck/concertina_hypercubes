...
This inductive calculation of the coordinates is realized in
[`old_to_new_points`](https://github.com/watchduck/concertina_hypercubes/blob/master/utils/functions.py).
A similar calculation of the formulas is realized in
[`old_to_new_formulas`](https://github.com/watchduck/concertina_hypercubes/blob/master/classes/functions_involving_formula_class.py).
Here the equivalent to point reflection is [`complement`](https://github.com/watchduck/concertina_hypercubes/blob/master/classes/formula.py).
...
The similarity of the 2-cocoon with the cube (just one edge more) is rather misleading.
But labeling its vertices from 0 to 7 like those of a cube (edge where bitwise&nbsp;&le;)
...
It checks if corresponding digits of two pseudo-octal strings are bitwise&nbsp;&le; (an edge of the cube)
or if they are 2&nbsp;and&nbsp;5 (the one edge of the 2-cocoon that is not in the cube).
The pseudo-octal string of a formula is generated by [`octal`](https://github.com/watchduck/concertina_hypercubes/blob/master/classes/formula.py).
An explanation how it works can be found [here](https://en.wikiversity.org/wiki/Formulas_in_predicate_logic#Pairs).
